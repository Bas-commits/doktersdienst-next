...
describe('GET /api/urentelling', () => {
  ...
  it('returns aggregated rows for valid request', async () => {
    ...
    const body = res._json as {
      van: number;
      tot: number;
      rows: Array<{ naam: string; initials: string; color: string; dienst: number }>;
      details: Array<{ naam: string; categorie: string; uren: number }>;
    };
    ...
    expect(body.rows[0].dienst).toBe(1);
