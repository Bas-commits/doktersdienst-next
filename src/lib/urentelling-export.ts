...
export async function downloadUrentellingWorkbook(params: {
  ...
  const summarySheet = XLSX.utils.aoa_to_sheet([
    ...
    [],
    ['Naam', 'Achterwacht', 'Dienst', 'Extra dokter', 'Totaal'],
    ...params.rows.map((row) => [
      row.naam,
      row.achterwacht,
      row.dienst,
  ...
  detailsSheet['!cols'] = [{ wch: 36 }, { wch: 28 }, { wch: 18 }, { wch: 18 }, { wch: 10 }];
