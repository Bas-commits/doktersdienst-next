...
export default function LijstDeelnemersPage() {
  ...
  // Track which color input is open
  ...
  async function handleColorChange(deelnemerId: number, color: string) {
    // Optimistic update
    setDeelnemers((prev) =>
      prev.map((d) => (d.id === deelnemerId ? { ...d, color } : d))
    );
    try {
      ...
      const data = await res.json();
      if (data.error) {
        toast.error(`Kleur opslaan mislukt: ${data.error}`);
      }
    } catch {
  ...
  const showWaarneemgroepColumn = allWaarneemgroepen.length > 1 || (!isAdmin && deelnemers.some((d) => d.waarneemgroepen.length > 0));
      ...
      <div className="mx-auto max-w-4xl space-y-6 px-4 py-8">
        <Card>
          ...
          <CardContent>
            ...
            {!loading && !error && deelnemers.length === 0 && (
                  ...
                  <thead>
                    <tr className="border-b text-left text-muted-foreground">
                      ...
                      {showWaarneemgroepColumn && (
                        <th className="pb-2 font-medium">Waarneemgroepen</th>
                      )}
                    </tr>
                  </thead>
                  <tbody>
